...
class Maze {
 public:
  ...
  /// @brief serialize all wall states to a byte-per-cell buffer for EEPROM.
  /// Layout is index x*16+y (matches the injection convention); each byte packs
  /// N|E<<2|S<<4|W<<6 (2 bits each). Portable - does not rely on struct layout.
  void save_to(uint8_t *buf) const {
    int i = 0;
    for (int x = 0; x < MAZE_WIDTH; x++)
      for (int y = 0; y < MAZE_HEIGHT; y++) {
        WallInfo w = m_walls[x][y];
        buf[i++] = (uint8_t)((w.north & 3) | ((w.east & 3) << 2) |
                             ((w.south & 3) << 4) | ((w.west & 3) << 6));
      }
  }
  /// @brief restore all wall states from a save_to() buffer.
  void load_from(const uint8_t *buf) {
    int i = 0;
    for (int x = 0; x < MAZE_WIDTH; x++)
      for (int y = 0; y < MAZE_HEIGHT; y++) {
        uint8_t b = buf[i++];
        m_walls[x][y].north = (WallState)(b & 3);
        m_walls[x][y].east  = (WallState)((b >> 2) & 3);
        m_walls[x][y].south = (WallState)((b >> 4) & 3);
        m_walls[x][y].west  = (WallState)((b >> 6) & 3);
      }
  }
  ...
  void initialise() {
    ...
    set_mask(MASK_OPEN);
    flood(goal());
    // NOTE (STM32 port): Serial warning removed. On overflow, m_flood_queue_overflow
    // is set - query flood_queue_overflow(). Will route via the CLI/UART once ported.
  }
  ...
  /// @brief unconditional wall set (both sides). Use to build a KNOWN maze
  ///        (ground truth / injected layout); NOT for exploration.
  void set_wall(const Location cell, const Heading heading, const WallState state) {
    set_wall_state(cell, heading, state);
  }
